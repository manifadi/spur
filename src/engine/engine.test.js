...
test('Level-Session spielt genau einen Teil: Original zuerst, Zähler "Teil X · Frage Y von Z"', () => {
  const s = initialState();
  const session = buildLessonSession(index, s, 'z1-umzug');
  assert.equal(session.part, 1);
  assert.equal(session.parts, 3);
  ...
  assert.ok(session.entries.every((e) => e.cardId.startsWith('z1-umzug-d:')));
  assert.deepEqual(session.entries.map((e) => e.feld), [1, 2, 3, 4].map((n) => ({ n, of: 4, part: 1, parts: 3 })));
});

test('Ringsegment = Teil: Teil geschafft füllt ein Segment, Level erst nach allen Teilen fertig', () => {
  const now = at('2026-09-23T10:00:00');
  let s = initialState();
  const parts = lessonParts(index, 'z1-buero');
  assert.equal(parts.length, 3);
  const [p1, p2] = parts;
  let r;
  for (const id of p1.cardIds.slice(0, -1)) s = commitAnswer(index, s, { cardId: id, mode: 'new' }, 'good', now).state;
  s = commitAnswer(index, s, { cardId: p1.cardIds.at(-1), mode: 'new' }, 'poor', now).state;
  assert.deepEqual(lessonSegments(index, s, 'z1-buero'), { total: 3, done: 0 });
  // Zweiter Versuch am Ende der Session meistert die Frage, ohne das Intervall zu ändern
  const before = s.cards[p1.cardIds.at(-1)].due;
  r = commitAnswer(index, s, { cardId: p1.cardIds.at(-1), mode: 'retry' }, 'good', now);
  s = r.state;
  assert.equal(s.cards[p1.cardIds.at(-1)].due, before);
  assert.equal(isPartDone(s, p1), true);
  assert.deepEqual(lessonSegments(index, s, 'z1-buero'), { total: 3, done: 1 });
  assert.equal(isLessonComplete(index, s, 'z1-buero'), false);
  // Nächstes Tippen startet Teil 2 – wieder mit Original
  const next = buildLessonSession(index, s, 'z1-buero', now);
  assert.equal(next.part, 2);
  assert.equal(next.entries[0].cardId, p2.cardIds[0]);
  assert.equal(next.entries[0].showSource, true);
});

test('Nicht geschaffter Teil startet komplett neu; Gemeistertes läuft als Übung ohne neue Intervalle', () => {
  const now = at('2026-09-23T10:00:00');
  let s = initialState();
  const [p1] = lessonParts(index, 'z1-buero');
  s = commitAnswer(index, s, { cardId: p1.cardIds[0], mode: 'new' }, 'good', now).state;
  s = commitAnswer(index, s, { cardId: p1.cardIds[1], mode: 'new' }, 'poor', now).state;
  const again = buildLessonSession(index, s, 'z1-buero', now);
  assert.equal(again.part, 1);
  assert.equal(again.entries.filter((e) => e.feld).length, p1.cardIds.length);
  assert.equal(again.entries[0].showSource, true);
  assert.equal(again.entries[0].mode, 'practice');
  assert.equal(again.entries[1].mode, 'new');
});
...
test('Pop-ups: ~60 % Chance, 3–5 Tage, höchstens eines pro Tag, Rest verschiebt sich', () => {
  ...
  let s = initialState();
  for (const id of index.lessons.get('l1-tiere').cardIds) s = commitAnswer(index, s, { cardId: id, mode: 'new' }, 'good', now).state;
...
test('Kapitelabschluss plant Pop-up; Migration alter Karten-IDs verliert nichts', () => {
  ...
  const ev = {};
  s = markLessonDone(index, s, 'l1-kueche', now, ev, () => 0.1);
...
test('Lektion fertig → nächster Knoten current, Karten werden fällig und erscheinen als due', () => {
  ...
  let s = initialState();
  let events;
  for (const id of index.lessons.get('z1-buero').cardIds) ({ state: s, events } = commitAnswer(index, s, { cardId: id, mode: 'new' }, 'good', d1));
  assert.equal(events.lessonDone, 'z1-buero');
  ...
  assert.equal(dueCardIds(index, s, d2).length, index.lessons.get('z1-buero').cardIds.length);
  const due = buildDueSession(index, s, 'z1-buero', d2);
...
test('Getrenntes Scheduling: Zuhören und Lesen haben je einen eigenen aktuellen Knoten', () => {
  ...
  assert.deepEqual(current, ['listen:z1-buero', 'read:l1-tiere']);
  // Zwei Zuhör-Lektionen erledigen: Lesen bleibt unberührt bei der ersten Lektion.
  for (const l of ['z1-buero', 'z1-umzug']) for (const id of index.lessons.get(l).cardIds) s = commitAnswer(index, s, { cardId: id, mode: 'new' }, 'good', d1).state;
  ...
  assert.deepEqual(cur, { listen: 'z1-berger', read: 'l1-tiere' });
